...
export async function POST(request: Request) {
  try {
    ...
    const { data: sermon, error: sermonError } = await client.from("expository_sermons").select("id, big_idea, purpose, manuscript, delivery_notes").eq("study_id", studyId).eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle(); if (sermonError) throw sermonError; if (!sermon) throw new Error("No sermon preparation exists for the selected Study.");
