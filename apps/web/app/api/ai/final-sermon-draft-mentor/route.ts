...
export async function POST(request: Request) {
  try {
    ...
    const { data: outline, error: outlineError } = await client.from("sermon_outline_points").select("heading, truth, text, explanation, illustration, application, transition").eq("sermon_id", sermon.id).order("position", { ascending: true }); if (outlineError) throw outlineError;
