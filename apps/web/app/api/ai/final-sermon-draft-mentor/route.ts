...
function errorStatus(message: string): number { if (/signed-in|session/i.test(message)) return 401; if (/Missing Supabase environment|not configured|Unsupported AI_PROVIDER/i.test(message)) return 503; return 502; }
export async function POST(request: Request) {
  try {
    const token = bearer(request); const { client, userId } = await context(token); const body = await request.json() as RequestBody; const studyId = requiredText(body.studyId, "Study ID");
    const { data: sermon, error: sermonError } = await client.from("expository_sermons").select("id, big_idea, purpose, manuscript, teaching_plan_id").eq("study_id", studyId).eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle(); if (sermonError) throw sermonError; if (!sermon) throw new Error("No sermon preparation exists for the selected Study.");
    const manuscript = requiredText(sermon.manuscript, "Saved manuscript");
    const { data: outlineRows, error: outlineError } = await client.from("sermon_outline_points").select("heading, truth, text, explanation, illustration, application, transition").eq("sermon_id", sermon.id).order("position", { ascending: true }); if (outlineError) throw outlineError;
