...
interface RequestBody {
  ...
  observationIds?: unknown; interpretationIds?: unknown; evidenceIds?: unknown; applicationIds?: unknown; biblicalTheologyIds?: unknown;
}
...
function ids(value: unknown): string[] { return Array.isArray(value) ? value.filter((item): item is string => typeof item === "string" && Boolean(item.trim())).map((item) => item.trim()) : []; }
...
function validateIds(requested: string[], returned: readonly { id: string }[]): boolean { const available = new Set(returned.map((row) => row.id)); return requested.every((id) => available.has(id)); }
export async function POST(request: Request) {
  try {
    const token = bearer(request); const { client, userId } = await context(token); const body = await request.json() as RequestBody;
    ...
    if (studyError) throw studyError; if (!study) throw new Error("The selected Study could not be found.");
    const observationIds = ids(body.observationIds); const interpretationIds = ids(body.interpretationIds); const evidenceIds = ids(body.evidenceIds); const applicationIds = ids(body.applicationIds); const biblicalTheologyIds = ids(body.biblicalTheologyIds);
    ...
    const [observationsResult, interpretationsResult, evidenceResult, applicationsResult, theologyResult] = await Promise.all([
      ...
      client.from("interpretation_evidence").select("id, description, evidence_type").eq("study_id", studyId).in("id", evidenceIds.length ? evidenceIds : [missingId]),
    ...
    const byId = <T extends { id: string }>(rows: readonly T[], selectedIds: readonly string[]) => { const map = new Map(rows.map((row) => [row.id, row])); return selectedIds.map((id) => map.get(id)).filter((row): row is T => Boolean(row)); };
