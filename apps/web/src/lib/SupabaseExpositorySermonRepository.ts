...
type DatabaseSermonRow = Database["public"]["Tables"]["expository_sermons"]["Row"];
type DatabaseOutlinePointRow = Database["public"]["Tables"]["sermon_outline_points"]["Row"];
...
export class SupabaseExpositorySermonRepository implements ExpositorySermonRepository {
    ...
    private async hydrate(row: DatabaseSermonRow): Promise<ExpositorySermon> {
        ...
        for (const rawPoint of outlineRows ?? []) { const point = rawPoint as unknown as DatabaseOutlinePointRow; const pointId = point.id as `${string}-${string}-${string}-${string}-${string}`; sermon.addOutlinePoint(point.heading, point.truth, { supportingObservationIds: Array.isArray(point.supporting_observation_ids) ? point.supporting_observation_ids.filter((value): value is string => typeof value === "string") : [], supportingInterpretationIds: Array.isArray(point.supporting_interpretation_ids) ? point.supporting_interpretation_ids.filter((value): value is string => typeof value === "string") : [], supportingEvidenceIds: Array.isArray(point.supporting_evidence_ids) ? point.supporting_evidence_ids.filter((value): value is string => typeof value === "string") : [], supportingApplicationIds: Array.isArray(point.supporting_application_ids) ? point.supporting_application_ids.filter((value): value is string => typeof value === "string") : [], supportingBiblicalTheologyIds: point.supporting_biblical_theology_ids ?? [] }, pointId, { text: point.text ?? "", explanation: point.explanation ?? "", illustration: point.illustration ?? "", application: point.application ?? "", transition: point.transition ?? "", textObservationIds: point.text_observation_ids ?? [], meaningInterpretationIds: point.meaning_interpretation_ids ?? [], meaningEvidenceIds: point.meaning_evidence_ids ?? [], responseApplicationIds: point.response_application_ids ?? [] }); }
        return sermon;
    }
}
