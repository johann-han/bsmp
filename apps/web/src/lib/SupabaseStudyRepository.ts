...
export class SupabaseStudyRepository implements StudyRepository {
    public async find(id: StudyId): Promise<StudySession | undefined> {
        ...
        const { data: study, error } = await supabase.from("studies").select("*").eq("id", id.toString()).eq("user_id", user.id).maybeSingle();
    ...
    public async findAll(): Promise<readonly StudySession[]> {
        const user = await this.requireUser();
        const { data: studies, error } = await supabase.from("studies").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
        if (error) throw error;
    ...
    public async save(study: StudySession): Promise<void> {
        ...
        const { error: studyError } = await supabase.from("studies").upsert({
        ...
        if (study.observations.length > 0) {
            const { error } = await supabase.from("study_observations").upsert(study.observations.map((observation) => ({
        ...
        if (study.interpretations.length > 0) {
            const { error: interpretationError } = await supabase.from("study_interpretations").upsert(study.interpretations.map((interpretation) => ({
            ...
            const { error: deleteLinksError } = await supabase.from("interpretation_observations").delete().in("interpretation_id", interpretationIds);
            ...
            if (supportLinks.length > 0) {
                const { error } = await supabase.from("interpretation_observations").upsert(supportLinks, { onConflict: "interpretation_id,observation_id" });
                if (error) throw error;
            }
            ...
            if (evidenceRows.length > 0) {
                const { error } = await supabase.from("interpretation_evidence").upsert(evidenceRows);
                if (error) throw error;
            }
        }

        if (study.applications.length > 0) {
            const { error } = await supabase.from("study_applications").upsert(study.applications.map((application) => ({
    ...
    public async delete(id: StudyId): Promise<void> {
        const user = await this.requireUser();
        const { error } = await supabase.from("studies").delete().eq("id", id.toString()).eq("user_id", user.id);
        if (error) throw error;
    }

    private async hydrateStudy(row: DatabaseStudyRow): Promise<StudySession> {
        const passageService = createStudyPassage();
        const { data: observationRows, error: observationError } = await supabase.from("study_observations").select("*").eq("study_id", row.id).order("created_at", { ascending: true });
        if (observationError) throw observationError;
        const { data: interpretationRows, error: interpretationError } = await supabase.from("study_interpretations").select("*").eq("study_id", row.id).order("created_at", { ascending: true });
        if (interpretationError) throw interpretationError;
        const { data: evidenceRows, error: evidenceError } = await supabase.from("interpretation_evidence").select("*").eq("study_id", row.id).order("created_at", { ascending: true });
        if (evidenceError) throw evidenceError;
        const { data: applicationRows, error: applicationError } = await supabase.from("study_applications").select("*").eq("study_id", row.id).order("created_at", { ascending: true });
        ...
        for (const interpretationRow of interpretationRows ?? []) {
            const { data: supportRows, error: supportError } = await supabase.from("interpretation_observations").select("observation_id").eq("interpretation_id", interpretationRow.id);
