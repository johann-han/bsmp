...
interface ResearchResult { answer: string; textualBasis: string[]; furtherQuestions: string[]; cautions: string[]; model: string; provider: string; }
...
export function BiblicalResearchWorkspace({ studyId }: Props) {
    ...
    async function runResearch() {
        ...
        try {
            ...
            const response = await fetch("/api/ai/biblical-research", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ studyId, question: question.trim() }) });
            ...
            setResult({ answer: payload.answer ?? "", textualBasis: payload.textualBasis ?? [], furtherQuestions: payload.furtherQuestions ?? [], cautions: payload.cautions ?? [], model: payload.model ?? "", provider: payload.provider ?? "" });
        } catch (reason) {
    ...
    return <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px", display: "grid", gap: 18 }}>
        ...
        <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginTop: 12 }}><button type="button" onClick={() => void runResearch()} disabled={running}>{running ? "Researching..." : "Investigate Question"}</button>{error && <span style={{ color: "#b91c1c" }}>{error}</span>}</div>
        </section>
