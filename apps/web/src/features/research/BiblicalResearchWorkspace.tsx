...
export function BiblicalResearchWorkspace({ studyId }: Props) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const [obs, ints, bt, sources] = await Promise.all([
    ...
    async function saveSources(sources: ResearchSource[], fallbackUrls: string[], userId: string) {
        const candidates = sources.length ? sources : fallbackUrls.map((url) => ({ url, title: fallbackTitle(url) }));
        if (!candidates.length) return;
        const rows = Array.from(new Map(candidates.map((source) => [source.url, source])).values()).slice(0, 10).map((source) => ({
            user_id: userId,
            study_id: studyId,
            url: source.url,
        ...
        const { error: saveError } = await supabase.from("research_sources").upsert(rows, { onConflict: "user_id,url" });
        if (saveError) throw saveError;
        setSourceMessage(`${rows.length} external source${rows.length === 1 ? "" : "s"} saved to your Research Source Library.`);
        void load();
    }
    ...
    async function runResearch() {
        ...
        if (contextualResearch && !effectiveExternal) { setError("Contextual research requires external sources."); return; }
        setRunning(true);
        try {
            const session = await supabase.auth.getSession();
            const token = session.data.session?.access_token;
            const userId = session.data.session?.user.id;
            if (!token || !userId) throw new Error("A signed-in user is required.");
    ...
    return <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px", display: "grid", gap: 18 }}>
        <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", fontSize: 13, color: "#4b5563" }}><span>{observations} observations</span><span>{interpretations} interpretations</span><span>{theology} theological syntheses</span><span>{savedSources.length} saved research sources</span></div>
        ...
        <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
            ...
            {effectiveExternal && <div style={{ marginTop: 12, padding: 14, border: "1px solid #e5e7eb", borderRadius: 10 }}>
                ...
                {savedSources.length ? <div style={{ display: "grid", gap: 8, maxHeight: 220, overflowY: "auto" }}>
                    {savedSources.map((source) => <label key={source.id} style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13 }}><input type="checkbox" checked={selectedSavedSourceIds.includes(source.id)} onChange={(event) => setSelectedSavedSourceIds((current) => event.target.checked ? [...current, source.id] : current.filter((id) => id !== source.id))} /><span style={{ flex: 1 }}><strong>{source.title}</strong><br /><span style={{ color: "#6b7280" }}>{source.url}{source.study_id === studyId ? " · This Study" : " · Research Library"}</span></span><button type="button" onClick={() => void removeSavedSource(source.id)} style={{ padding: "2px 7px" }}>Remove</button></label>)}
