            ...
            <div style={{ display: "grid", gap: 20 }}>
                ...
                {sermon.outline.length === 0 ? (
                    ...
                    sermon.outline.map((point, index) => {
                        ...
                        const firstTarget = observations[0] ? `observation-${observations[0].id.value}`
                            : interpretations[0] ? `interpretation-${interpretations[0].id.value}`
                            : evidence[0] ? `evidence-${evidence[0].id.value}`
                            : applications[0] ? `application-${applications[0].id.value}`
                            : undefined;
                        ...
                        return (
                            <section key={point.id} style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
                                    ...
                                    <WorkspaceLink study={study} href={workspaceHref(studyId, firstTarget)}>Open Study Workspace</WorkspaceLink>
                                </div>

                                <section style={{ marginTop: 16, border: "1px solid #e5e7eb", borderRadius: 10, padding: 16, background: "#f8fafc" }}>
                                    <h3 style={{ marginTop: 0 }}>Study Support</h3>
                                    {firstTarget ? (
