                ...
                <div className="bsmp-delivery-print-page">
                    ...
                    <main className="bsmp-delivery-print-main">
                        {focus === "manuscript" ? <div className="bsmp-delivery-manuscript" style={{ fontSize: readingConfig.manuscript, lineHeight: 1.7 }}>
                            ...
                            {hasTraceableSections ? sections.map((section) => <article id={`delivery-section-${encodeURIComponent(section.id)}`} className="bsmp-delivery-print-section" key={section.id} style={{ fontSize: readingConfig.manuscript }}><h2 className="bsmp-delivery-section-heading" style={{ fontSize: readingConfig.heading }}>{section.title}</h2><div className="bsmp-delivery-section-content">{splitParagraphs(section.content).map((paragraph, index) => { const labeled = splitLabeledBlock(paragraph); return labeled ? <p className="bsmp-delivery-content-block" key={`${section.id}-${index}`}><span className="bsmp-delivery-content-block-label">{labeled.label}</span><span className="bsmp-delivery-content-block-body">{labeled.body}</span></p> : <p className="bsmp-delivery-content-paragraph" key={`${section.id}-${index}`}>{paragraph}</p>; })}</div></article>) : paragraphs.map((paragraph, index) => <p className="bsmp-delivery-content-paragraph" key={index}>{paragraph}</p>)}
