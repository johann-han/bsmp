...
export function FinalSermonDraftMentorPanel({ studyId }: { studyId: string }) {
  ...
  if (loading) return <section className="bsmp-print-hide" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff", marginTop: 20 }}><strong>Final Sermon Draft Mentor</strong><p>Loading saved manuscript...</p></section>;
  if (!sermon) return <section className="bsmp-print-hide" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff", marginTop: 20 }}><h2 style={{ marginTop: 0 }}>Final Sermon Draft Mentor</h2><p style={{ color: "#6b7280" }}>{error ?? "No sermon preparation is available."}</p></section>;
  ...
  return <section className="bsmp-print-hide bsmp-final-sermon-mentor" style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff", marginTop: 20 }}>
    ...
    <div className="bsmp-final-sermon-mentor-meta" style={{ marginTop: 12, display: "flex", gap: 14, flexWrap: "wrap", fontSize: 13, color: "#6b7280" }}><span><strong>{wordCount}</strong> words saved</span><span>{sermon.outline.length} prepared outline point{sermon.outline.length === 1 ? "" : "s"}</span><span>{sermon.bigIdea ? "Big Idea defined" : "Big Idea missing"}</span><span>{sermon.purpose ? "Purpose defined" : "Purpose missing"}</span></div>
    ...
    {result && <div className="bsmp-final-sermon-mentor-result" style={{ marginTop: 16, padding: 16, borderRadius: 10, background: "#f8fafc", border: "1px solid #e5e7eb" }}><div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}><strong>Assessment: {labels[result.assessment]}</strong>{result.focuses.map((focus) => <span key={focus} style={{ padding: "3px 8px", borderRadius: 999, background: "#e5e7eb", fontSize: 12 }}>{focusLabels[focus] ?? focus}</span>)}</div><p style={{ margin: "12px 0" }}>{result.coaching}</p><div style={{ fontSize: 12, color: "#6b7280" }}>Provider: {result.provider} · Model: {result.model}</div><button type="button" onClick={reload} style={{ marginTop: 12, padding: "8px 12px", minHeight: 42 }}>Reload Saved Manuscript</button></div>}
  </section>;
}
