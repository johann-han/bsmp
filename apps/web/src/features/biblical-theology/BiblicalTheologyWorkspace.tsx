...
export function BiblicalTheologyWorkspace({ studyId }: Props) {
    ...
    async function save() {
        setError(null);
        setMessage(null);
        if (!theme.trim() || !synthesis.trim()) {
            setError("Enter a theme and synthesis before saving.");
            return;
        }
        if (selected.length === 0) {
            setError("Select at least one supporting interpretation before saving.");
            return;
        }
        setSaving(true);
        try {
            ...
            if (editingId) {
                const { error: updateError } = await supabase.from("biblical_theology_entries").update({
                    theme: theme.trim(),
                    synthesis: synthesis.trim(),
                    interpretation_ids: selected,
                }).eq("id", editingId).eq("study_id", studyId);
                if (updateError) throw updateError;
                setMessage("Biblical Theology entry updated.");
            } else {
                const { error: insertError } = await supabase.from("biblical_theology_entries").insert({
    ...
    async function remove(id: string) {
        ...
        try {
            ...
            setError(reason instanceof Error ? reason.message : "Unable to delete Biblical Theology entry.");
        }
    }
    ...
    function edit(entry: Entry) {
        ...
        setSelected([...entry.interpretation_ids]);
        setMessage(null);
    }
    ...
    return (
        <div style={{ display: "grid", gap: 18 }}>
            <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
                <div style={{ fontSize: 13, color: "#6b7280" }}>Study → Biblical Theology</div>
            ...
            <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
                ...
                <div style={{ display: "grid", gap: 12 }}>
                    ...
                    <div>
                        <strong>Supporting interpretations</strong>
                        <p style={{ margin: "4px 0 8px", color: "#6b7280", fontSize: 13 }}>Select the interpretations that establish this synthesis.</p>
            ...
            <section style={{ border: "1px solid #ddd", borderRadius: 12, padding: 20, background: "#fff" }}>
                ...
                {entries.length === 0 ? <p style={{ color: "#6b7280" }}>No Biblical Theology syntheses recorded yet.</p> : <div style={{ display: "grid", gap: 12 }}>{entries.map((entry) => <article key={entry.id} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 14 }}><strong>{entry.theme}</strong><p style={{ whiteSpace: "pre-wrap" }}>{entry.synthesis}</p><div style={{ fontSize: 13, color: "#6b7280" }}>Traceable to {entry.interpretation_ids.length} interpretation{entry.interpretation_ids.length === 1 ? "" : "s"}.</div><div style={{ marginTop: 10, display: "flex", gap: 8 }}><button type="button" onClick={() => edit(entry)}>Edit</button><button type="button" onClick={() => void remove(entry.id)}>Delete</button></div></article>)}</div>}
            </section>
        </div>
    );
}
